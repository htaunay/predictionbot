...
var extractOdds = function(html) {
    ...
    var firstSlice = html.split('Palpite do Bing: </span>');
    if(firstSlice.length <= 1)
        return null;

    var odds = firstSlice[1].split('</div>')[0];
    return odds;
};
...
var getPrediction = function(team, cb) {
    ...
    request(query, function (error, response, body) {
        ...
        if (!error && response.statusCode == 200) {
            ...
            if(odds === null) cb('Not quite sure yet.. try again later.');
            else cb(buildMessage(odds));
        }
        else {
